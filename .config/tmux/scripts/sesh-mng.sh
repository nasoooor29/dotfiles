...
sesh connect "$(
    sesh list -t | grep -v $TERM_NAME | fzf-tmux -p 60%,40% --border --margin=0 \
        --header '  A-1 tmux  A-2 zoxide  A-3 My Repos  A-4 find  A-d kill ' \
        --no-sort --ansi --border-label ' sesh ' --prompt '  ' --border=rounded \
        --bind "tab:down,btab:up" \
        --bind "alt-1:change-prompt(  )+reload(sesh list -t | grep -v $TERM_NAME)" \
        ...
        --bind "alt-d:execute(tmux kill-session -t {})+change-prompt(  )+reload(sesh list -t | grep -v $TERM_NAME)" \
